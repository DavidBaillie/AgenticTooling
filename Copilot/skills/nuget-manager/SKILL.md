...
1.  **NEVER** directly edit `.csproj`, `.props`, or `Directory.Packages.props` files to **add** or **remove** packages. Always use `dotnet add package` and `dotnet remove package` commands. If the developer has added entries to one of these file types, use the `dotnet restore` command to resolve dependencies.
...
3.  **VERSION UPDATES** must follow the mandatory workflow:
    ...
    - All versions are managed centrally (`Directory.Packages.props`).
